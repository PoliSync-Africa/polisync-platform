...
const { checkElectoralGeographyIntegrity, EXPECTED_CONSTITUENCIES_BY_REGION } = require("../services/electoralGeographyIntegrityService");
...
exports.report = async (req, res) => {
  try {
    ...
    const byRegion = new Map(regions.map((region) => [region._id.toString(), {
      ...
      name: region.name,
      slug: region.slug,
      expectedConstituencyCount: EXPECTED_CONSTITUENCIES_BY_REGION[region.name] ?? null,
      constituencyCount: 0,
    ...
    const data = Array.from(byRegion.values()).map((row) => {
      const constituencyGap = row.expectedConstituencyCount === null ? null : row.constituencyCount - row.expectedConstituencyCount;
      ...
      const constituencyCountWrong = row.expectedConstituencyCount !== null && constituencyGap !== 0;
      return { ...row, constituencyGap, constituencyCountComplete: !constituencyCountWrong, status: hasIssue || constituencyCountWrong ? "Needs Review" : "Healthy" };
    });
    ...
    return res.json({
      ...
      data: {
        ...
        expectedRegions: 16,
        expectedConstituencies: 276,
        activeRegions: regions.length,
        activeConstituencies: constituencies.length,
