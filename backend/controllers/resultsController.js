...
exports.submitResult = async (req, res) => {
  try {
    ...
    // Authorization is based on the verified organization membership attached by
    // routes/results.js. Never trust a client-supplied role or mutate the JWT role.
    const membership = req.pollingStationMembership || await OrganizationMembership.findOne({
      userId: req.user._id,
      role: "polling_station_agent",
      pollingStationId,
      status: "approved",
    }).lean();
    if (!membership || membership.role !== "polling_station_agent") {
      return res.status(403).json({ success: false, message: "You are not approved for this polling station." });
    }
...
exports.dashboard = async (req, res) => {
  try {
    ...
    results.forEach(result => {
      const org = result.organizationId; const election = result.electionId;
      ...
      if (election?.type) electionTypeSet.add(election.type); if (election?._id) electionMap.set(String(election._id), election);
      submittedStations += 1; totalVotes += result.manualTotals?.totalValidVotes || 0;
      if (result.verificationStatus === "verified") verified += 1; else if (["discrepancy", "disputed"].includes(result.verificationStatus)) discrepancy += 1; else if (result.verificationStatus === "rejected") rejected += 1; else pending += 1;
      const regionId = String(result.regionId?._id || result.regionId || ""); const regionName = result.regionId?.name || regionId || "Unassigned region";
      if (regionId) { if (!regions.has(regionId)) regions.set(regionId, { id: regionId, name: regionName, regionNumber: result.regionId?.regionNumber || null, submitted: 0, validVotes: 0, candidates: {} }); const region = regions.get(regionId); region.submitted += 1; region.validVotes += result.manualTotals?.totalValidVotes || 0; for (const c of result.candidateResults || []) region.candidates[c.candidateName] = (region.candidates[c.candidateName] || 0) + c.manualVotes; }
      const constituencyId = String(result.constituencyId?._id || result.constituencyId || "");
      if (constituencyId) { const constituencyName = result.constituencyId?.name || constituencyId; if (!constituencies.has(constituencyId)) constituencies.set(constituencyId, { id: constituencyId, name: constituencyName, constituencyNumber: result.constituencyId?.constituencyNumber || null, regionId, submitted: 0, validVotes: 0, candidates: {} }); const constituency = constituencies.get(constituencyId); constituency.submitted += 1; constituency.validVotes += result.manualTotals?.totalValidVotes || 0; for (const c of result.candidateResults || []) constituency.candidates[c.candidateName] = (constituency.candidates[c.candidateName] || 0) + c.manualVotes; }
