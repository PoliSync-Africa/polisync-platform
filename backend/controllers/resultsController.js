...
// One polling-station submission becomes the source of truth for
// station -> constituency -> region -> party aggregation.
exports.submitResult = async (req, res) => {
  try {
    ...
    if (!station || !station.isActive) {
      return res.status(404).json({ success: false, message: "Polling station not found or inactive." });
    }
    ...
    if (existing) {
      return res.status(409).json({ success: false, message: "A result has already been submitted for this polling station and election.", resultId: existing._id });
    }
    ...
    // Return all hierarchy identifiers so receiving dashboards can update immediately.
    res.status(201).json({
      success: true,
      message: verificationStatus === "verified" ? "Result submitted and verified." : "Result submitted successfully.",
      result,
      propagation: {
...
exports.dashboard = async (req, res) => {
  try {
    const match = {};
    if (req.user.role === "polling_station_agent") match.submittedBy = req.user._id;
    const results = await Result.find(match).lean();
    const national = {}, regions = {}, constituencies = {};
    let totalVotes = 0;
    results.forEach(result => {
      totalVotes += result.manualTotals?.totalValidVotes || 0;
      for (const c of result.candidateResults || []) {
        national[c.candidateName] = (national[c.candidateName] || 0) + c.manualVotes;
        const regionKey = String(result.regionId);
        const constituencyKey = String(result.constituencyId);
        regions[regionKey] ||= {};
        constituencies[constituencyKey] ||= {};
        regions[regionKey][c.candidateName] = (regions[regionKey][c.candidateName] || 0) + c.manualVotes;
        constituencies[constituencyKey][c.candidateName] = (constituencies[constituencyKey][c.candidateName] || 0) + c.manualVotes;
      }
    });
    res.json({ success: true, pollingStationsReported: results.length, totalVotes, national, regions, constituencies });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.getResults = async (req, res) => {
  try {
    const results = await Result.find().populate("submittedBy", "fullName role email phoneNumber").sort({ createdAt: -1 });
