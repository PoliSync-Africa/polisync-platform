...
exports.getGeographicAssignments = async (req, res) => {
  try {
    ...
    const membershipQuery = {
      status: "approved",
      level: { $in: ["regional", "constituency", "polling_station"] },
    };
    if (regionId && regionId !== "all") membershipQuery.regionId = regionId;
    if (constituencyId && constituencyId !== "all") membershipQuery.constituencyId = constituencyId;
    if (pollingStationId && pollingStationId !== "all") membershipQuery.pollingStationId = pollingStationId;
    ...
    const resultQuery = {};
    if (electionId && electionId !== "all") resultQuery.electionId = electionId;
    ...
    const assignments = memberships
      ...
      .map((m) => ({
        ...
        organization: m.organizationId
          ? { id: m.organizationId._id, name: m.organizationId.name || m.organizationId.politicalPartyName || "Organization" }
          : null,
        ...
        pollingStation: m.pollingStationId
          ? { id: m.pollingStationId._id, name: m.pollingStationId.name, code: m.pollingStationId.pollingStationCode }
          : null,
      }));
