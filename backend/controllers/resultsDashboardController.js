...
exports.dashboard = async (req, res) => {
  try {
    ...
    const elections = await Election.find(electionFilter).select("name year type country status").sort({ year: -1, createdAt: -1 }).lean();
    ...
    for (const result of results) {
      ...
      if (rKey) {
        ...
        const item = regionTotals.get(rKey); item.submitted += 1; item.validVotes += Number(result.manualTotals?.totalValidVotes || 0); addCandidateTotals(item.candidates, result.candidateResults);
      }
      ...
      if (cKey) {
        ...
        const item = constituencyTotals.get(cKey); item.submitted += 1; item.validVotes += Number(result.manualTotals?.totalValidVotes || 0); addCandidateTotals(item.candidates, result.candidateResults);
      }
    ...
    const constituencies = await Constituency.find(constituencyFilter).select("name constituencyNumber district regionId").populate("regionId", "name regionNumber").sort({ regionId: 1, constituencyNumber: 1, name: 1 }).lean();
