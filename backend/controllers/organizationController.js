...
exports.createOrganizationRequest = async (req, res) => {
  try {
    ...
    let candidateParty = null, candidateIsIndependent = false, candidateRegion = null, candidateConstituency = null, candidatePartyLogo = "", selectedElection = null;
    if (isCandidate) {
      ...
      // Direct presidential registration is Independent-only. Political-party
      // presidential candidates are managed from their own party dashboard.
      if (organizationType === "presidential_candidate" && !candidateIsIndependent) return res.status(403).json({ success: false, code: "PARTY_CANDIDATE_USE_PARTY_DASHBOARD", message: "Presidential candidates for political parties must be created and maintained inside the respective political party dashboard. Direct presidential registration is only available for Independent candidates." });
    ...
    return res.status(201).json({ success: true, message: isCandidate ? "Candidate registration submitted for the selected election. Your photo and candidate details are awaiting the required approval." : "Organization request submitted. Super Admin approval is required before the organization becomes active.", organization: { id: organization._id, name: organization.name, slug: organization.slug, organizationType: organization.organizationType, organizationStatus: organization.organizationStatus, candidateParty: organization.candidateParty, candidateIsIndependent: organization.candidateIsIndependent, electionId: organization.electionId, electionName: selectedElection?.name || null, candidatePartyLogo, region: organization.region, constituency: organization.constituency }, nextStep: "Candidate approval is required before publication in the election." });
...
exports.getAllOrganizations = async (req, res) => { try { const admin = await requireSuperAdmin(req, res); if (!admin) return; const organizations = await Organization.find({}).sort({ createdAt: -1 }).lean(); const organizationIds = organizations.map((item) => item._id); const membershipCounts = await OrganizationMembership.aggregate([{ $match: { organizationId: { $in: organizationIds }, status: "approved" } }, { $group: { _id: "$organizationId", members: { $sum: 1 }, admins: { $sum: { $cond: [{ $in: ["$role", ADMIN_ROLES] }, 1, 0] } } } }]); const countMap = new Map(membershipCounts.map((item) => [String(item._id), item])); const enriched = organizations.map((o) => ({ ...o, memberCount: (countMap.get(String(o._id)) || {}).members || 0, adminCount: (countMap.get(String(o._id)) || {}).admins || 0 })); return res.json({ success: true, organizations: enriched, totals: { organizations: enriched.length, active: enriched.filter((i) => i.organizationStatus === "approved").length, pending: enriched.filter((i) => i.organizationStatus === "pending").length, suspended: enriched.filter((i) => i.organizationStatus === "suspended").length, politicalParties: enriched.filter((i) => i.organizationType === "political_party").length, observers: enriched.filter((i) => i.organizationType === "observer_organization").length } }); } catch (error) { return res.status(500).json({ success: false, message: error.message || "Unable to retrieve organizations." }); } };
