...
exports.approveCandidateBySuperAdmin = async (req, res) => {
  try {
    ...
    if (!party) return res.status(400).json({ success: false, message: "The candidate's approved political party or Independent participant could not be found." });
    const hasAdmin = await requireNationalAdmin(party._id);
    if (hasAdmin) return res.status(409).json({ success: false, code: "PARTY_ADMIN_REQUIRED", message: "This participant has an approved National Party Admin. The National Admin must approve this candidate." });
