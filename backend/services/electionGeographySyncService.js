...
async function synchronizeElectionParties(election) {
  ...
  const parties = organizations.map((org) => {
...
async function synchronizeElectionGeography(electionId) {
  ...
  const snapshot = await ElectionGeographySnapshot.findOneAndUpdate({ electionId }, { $set: { source: "PoliSync official electoral geography", sourceYear: pollingStations.reduce((max, s) => Math.max(max, Number(s.sourceYear) || 0), 0) || null, synchronizedAt: new Date(), regions: regions.length, constituencies: validConstituencies.length, pollingStations: validStations.length, geographyVersion: "current", status: "synchronized", error: "" } }, { upsert: true, new: true, setDefaultsOnInsert: true });
