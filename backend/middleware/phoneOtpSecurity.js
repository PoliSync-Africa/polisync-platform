...
const isPhoneVerificationValid = (user) => {
  ...
  if (!user.lastPhoneVerificationAt) {
    return false;
  }

  const lastVerification = new Date(user.lastPhoneVerificationAt).getTime();

  if (Number.isNaN(lastVerification)) {
    return false;
  }

  return Date.now() - lastVerification < PHONE_VERIFICATION_WINDOW_MS;
};
...
const createLoginOtpChallenge = async (user) => {
  ...
  if (!user.phone) {
    throw new Error("A registered phone number is required.");
  }
  ...
  user.loginOtpChallengeHash = challengeHash;
...
const startLoginOtpChallenge = async (user) => {
  ...
  return {
    ...
    expiresIn: Math.floor(LOGIN_OTP_EXPIRY_MS / 1000),
...
const verifyLoginOtp = async ({ userId, challengeId, code }) => {
  if (!userId || !challengeId || !code) {
    return {
      ...
      message: "User ID, challenge ID and OTP are required.",
    };
  }
  ...
  if (
    ...
    return {
      ...
      message: "The login verification challenge is invalid.",
    };
  }
  ...
  try {
    ...
    return {
      ...
      message: "Unable to verify the phone code at this time.",
    };
  }
  ...
  if (!verificationResult || verificationResult.success !== true) {
    ...
    const remainingAttempts = Math.max(
      0,
      MAX_LOGIN_OTP_ATTEMPTS - user.loginOtpAttempts
    );
  ...
  return {
    ...
    code: "PHONE_OTP_VERIFIED",

    message: "Phone verification successful.",

    userId: user._id,

    lastPhoneVerificationAt: user.lastPhoneVerificationAt,
  };
};
...
const resendLoginOtp = async ({ userId, challengeId }) => {
  ...
  if (user.loginOtpAttempts >= MAX_LOGIN_OTP_ATTEMPTS) {
    ...
    return {
      ...
      message: "Please start a new login verification request.",
    };
  }
