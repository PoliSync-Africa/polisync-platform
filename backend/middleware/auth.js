...
const protect = async (req, res, next) => {
  try {
    ...
    if (!decoded.userId || typeof decoded.userId !== "string" || !decoded.sessionId || typeof decoded.sessionId !== "string") return res.status(401).json({ success: false, code: "SESSION_INVALID", message: "This security session is no longer valid. Please log in again." });
