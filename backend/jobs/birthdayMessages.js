...
async function processBirthdayMessages() {
  ...
  const users = await User.find({
    ...
    .select("_id firstName displayName phone dateOfBirth")
    .lean();
  ...
  for (const user of users) {
    ...
    const existing = await BirthdayMessageLog.findOne({
      user: user._id,
      birthdayDate,
    });
    ...
    // Always personalize the birthday message with the user's first name.
    const firstName = String(
      user.firstName || user.displayName || "there"
    ).trim();

    const message = birthdayMessage(firstName);
    ...
    try {
      ...
      const logData = {
        ...
        status: "sent",
        provider: "arkesel",
        providerCode: result.response?.code
          ? String(result.response.code)
          : null,
        providerMessage:
          result.response?.message || result.response?.status || null,
        sentAt: new Date(),
        error: null,
      };

      if (existing) {
        await BirthdayMessageLog.updateOne(
          { _id: existing._id },
          { $set: logData }
        );
      } else {
        await BirthdayMessageLog.create(logData);
      }
      ...
      try {
        ...
        console.error("Birthday log error:", logError.message);
      }

      console.error(
        `Birthday SMS failed for user ${user._id}:`,
        error.message
      );
    }
  }
  ...
  console.log(
    `🎂 Birthday job ${birthdayDate}: sent=${sent}, skipped=${skipped}, failed=${failed}`
  );
...
function startBirthdayJob() {
  const isSevenAM = () => {
    ...
    return (
      Number(parts.hour) === JOB_HOUR &&
      Number(parts.minute) === JOB_MINUTE
    );
  };
  ...
  const timer = setInterval(async () => {
    ...
    // Prevent duplicate execution during the 7:00 AM minute.
  ...
  console.log(
    "🎂 Birthday automation enabled: daily at 07:00 Africa/Accra."
  );
