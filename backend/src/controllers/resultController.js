...
exports.submitResult = async (req, res) => {
  try {
    ...
    if (!objectId(electionId) || !objectId(pollingStationId) || !Array.isArray(candidateResults) || !candidateResults.length) return res.status(400).json({ success: false, message: "Election, polling station and candidate results are required." });
    const [station, election, context] = await Promise.all([PollingStation.findById(pollingStationId).lean(), Election.findById(electionId).lean(), getAccessContext(req)]);
    ...
    const result = await Result.create({
      organizationId, electionId, pollingStationId, regionId: station.regionId, constituencyId: station.constituencyId,
      pollingStationCode: station.pollingStationCode || station.code || String(station._id), submittedBy: req.user._id,
      candidateResults: candidateResults.map((candidate) => ({ candidateId: String(candidate.candidateId || ""), candidateName: String(candidate.candidateName || "").trim(), party: String(candidate.party || "").trim(), manualVotes: asNumber(candidate.manualVotes ?? candidate.votes), pinkSheetVotes: candidate.pinkSheetVotes == null ? null : asNumber(candidate.pinkSheetVotes), comparisonStatus: candidate.comparisonStatus || "not_checked" })),
      manualTotals: { totalValidVotes: asNumber(manualTotals?.totalValidVotes), rejectedVotes: asNumber(manualTotals?.rejectedVotes), totalBallots: asNumber(manualTotals?.totalBallots) },
  ...
  } catch (error) { console.error("Submit result error:", error); return res.status(500).json({ success: false, message: error.message || "Unable to submit result." }); }
};

exports.getElectionResults = async (req, res) => {
  try {
    const { filter } = await buildResultFilter(req, { electionId: req.params.electionId });
    const results = await Result.find(filter).populate("pollingStationId", "code pollingStationCode name country regionId constituencyId").populate("submittedBy", "firstName lastName platformRole").sort({ createdAt: -1 }).lean();
...
exports.getResults = async (req, res) => {
  try {
    ...
    const results = await Result.find(filter).populate("pollingStationId", "code pollingStationCode name country regionId constituencyId").populate("submittedBy", "firstName lastName platformRole").populate("electionId", "name year type country status").sort({ createdAt: -1 }).lean();
    return res.json({ success: true, count: results.length, data: results });
  } catch (error) { return res.status(500).json({ success: false, message: error.message }); }
};
...
exports.dashboard = async (req, res) => {
  try {
    ...
    let requestedElectionId = objectId(req.query?.electionId);
    const candidateElectionIds = (context.candidates || []).map((c) => c.electionId).filter(Boolean).map(String);

    // Candidate dashboards cannot drift into another election. If the UI starts
    // with a generic first election, silently anchor it to the candidate's real election.
    if (!context.isSuperAdmin && candidateElectionIds.length && (!requestedElectionId || !candidateElectionIds.includes(String(requestedElectionId)))) {
      requestedElectionId = objectId(candidateElectionIds[0]);
    }
    ...
    if (!effectiveElection) {
      const candidateElectionId = candidateElectionIds[0];
    ...
    if (!effectiveElection) {
      const electionFilter = context.isSuperAdmin ? {} : { $or: [...((context.memberships || []).map((m) => m.organizationId ? { organizationId: m.organizationId } : null).filter(Boolean)), { organizationId: null }] };
    ...
    if (!context.isSuperAdmin && context.candidates?.length) {
      ...
      if (candidateClauses.length) {
        ...
        filter.$and = [...(existingOr ? [{ $or: existingOr }] : []), { $or: candidateClauses }];
        delete filter.$or;
      }
    }
    ...
    let totalValidVotes = 0, rejectedVotes = 0, totalBallots = 0;
    ...
    for (const result of results) {
      ...
      const rid = String(result.regionId || ""), cid = String(result.constituencyId || "");
      if (rid) { const item = regionCoverage.get(rid) || { regionId: result.regionId, name: regionName.get(rid) || "Unknown region", received: 0 }; item.received += 1; regionCoverage.set(rid, item); }
      if (cid) { const item = constituencyCoverage.get(cid) || { constituencyId: result.constituencyId, name: constituencyName.get(cid) || "Unknown constituency", received: 0 }; item.received += 1; constituencyCoverage.set(cid, item); }
    ...
    const selectedStation = objectId(req.query?.pollingStationId), selectedConstituency = objectId(req.query?.constituencyId), selectedRegion = objectId(req.query?.regionId);
    ...
    if (!context.isSuperAdmin && context.memberships?.length) {
      const scopeOr = buildScopeOr(context).map((scope) => { const x = {}; if (scope.regionId) x.regionId = scope.regionId; if (scope.constituencyId) x.constituencyId = scope.constituencyId; if (scope.pollingStationId) x._id = scope.pollingStationId; return x; }).filter((x) => Object.keys(x).length);
    ...
    const coverage = { national: { totalPollingStations: nationalTotal, received: results.length, awaitingPending: Math.max(nationalTotal - results.length, 0), totalValidVotes, rejectedVotes, totalBallots }, regions: [], constituencies: [] };
    for (const item of regionCoverage.values()) { const total = await PollingStation.countDocuments({ regionId: item.regionId }); coverage.regions.push({ ...item, totalPollingStations: total, awaitingPending: Math.max(total - item.received, 0) }); }
    for (const item of constituencyCoverage.values()) { const total = await PollingStation.countDocuments({ constituencyId: item.constituencyId }); coverage.constituencies.push({ ...item, totalPollingStations: total, awaitingPending: Math.max(total - item.received, 0) }); }

    return res.json({ success: true, data: {
      scope: view, election: effectiveElection, coverage,
      ...
      results, totalResults: results.length, totalValidVotes, rejectedVotes, totalBallots, candidate: context.candidates?.[0] || null,
    } });
  } catch (error) { console.error("Results dashboard error:", error); return res.status(500).json({ success: false, message: error.message || "Unable to load election results." }); }
};
...
exports.verifyResult = async (req, res) => {
  try {
    ...
    result.verificationStatus = "verified"; result.verifiedBy = req.user._id; result.verifiedAt = new Date(); await result.save();
