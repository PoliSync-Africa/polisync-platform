...
export default function WorkspaceLab() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(e.message || "Unable to load the workspace lab.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const byType = useMemo(() => {
    const map = new Map();
    catalog.forEach((item) => map.set(item.organizationType, item));
    return map;
  }, [catalog]);
  ...
  return (
    <DashboardShell role="super_admin" navigation={superAdminNavigation} activeSection="workspace-lab" title="Workspace Lab" subtitle="Super Admin testing access to every organization workspace">
      <main className="page">
        <section className="hero">
          <div>
            <div className="eyebrow">SUPER ADMIN • PLATFORM QA</div>
            <h1>Open Every Workspace</h1>
            <p>Super Admin can enter organization workspaces in a controlled test mode even when no organization exists yet.</p>
          </div>
        ...
        <section className="notice">
          ...
          <span>Workspace preview does not grant organization members extra privileges. It gives the Super Admin a platform-level QA session so broken screens, workflows and APIs can be reproduced and repaired without creating fake organizations.</span>
        </section>
          ...
          {types.map(([type, title, description]) => {
            ...
            return (
              <article className="card" key={type}>
                ...
                <div className="copy">
                  <div className="type">{type.replace(/_/g, " ")}</div>
                  <h2>{title}</h2>
                  <p>{description}</p>
                </div>
                {existing ? <div className="status">LIVE ORGANIZATION AVAILABLE</div> : <div className="status sandbox">NO ORGANIZATION REQUIRED</div>}
                <div className="actions">
                  <button className="primary" onClick={() => openWorkspace(type, existing?.id || "")}>Open Workspace</button>
                  {existing && <button className="secondary" onClick={() => openWorkspace(type)}>Open Sandbox</button>}
                </div>
              </article>
            );
          })}
