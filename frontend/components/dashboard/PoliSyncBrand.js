    ...
    const search = (
      <div className="dashboard-search-portal" role="search">
        <div className="dashboard-search-row">
          {party && <PartyLogo party={party} size={38} className="dashboard-search-party-logo" />}
        ...
        <style jsx>{`
          .dashboard-search-portal { position:fixed; top:92px; right:18px; width:360px; z-index:1500; }
          .dashboard-search-row { display:flex; align-items:center; gap:8px; }
          .dashboard-search-party-logo { flex:0 0 auto; }
          ...
          .search-results { margin-top:6px; margin-left:46px; padding:7px; background:#fff; border:1px solid #dce6df; border-radius:13px; box-shadow:0 15px 35px rgba(7,45,25,.16); }
          ...
          @media (max-width:760px) { .dashboard-search-portal { top:76px; left:12px; right:12px; width:auto; } .search-results { margin-left:46px; } }
        `}</style>
      </div>
    );
