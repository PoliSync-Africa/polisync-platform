...
export default function ElectionGeographyAssignmentsView({ title = "Election Operations by Geography" }) {
  ...
  useEffect(() => {
    Promise.all([api("/api/elections"), api("/api/electoral-geography/regions")]).then(([e, g]) => {
      const list = Array.isArray(e.elections) ? e.elections : Array.isArray(e.data?.elections) ? e.data.elections : Array.isArray(e.data) ? e.data : [];
      const geography = Array.isArray(g.data) ? g.data : Array.isArray(g.data?.regions) ? g.data.regions : [];
      setElections(list);
      if (list[0]?._id || list[0]?.id) setElectionId(String(list[0]._id || list[0].id));
      setRegions(geography);
  ...
  useEffect(() => {
    ...
    api(`/api/electoral-geography/regions/${encodeURIComponent(regionId)}/constituencies`).then((j) => setConstituencies(Array.isArray(j.data) ? j.data : Array.isArray(j.data?.constituencies) ? j.data.constituencies : [])).catch((e) => setError(e.message));
  }, [regionId]);

  useEffect(() => { setPollingStationId("all"); }, [constituencyId]);
  ...
  return <section className="geo-assignments">
    ...
    <div className="filters">
      <label>Election<select value={electionId} onChange={(e) => setElectionId(e.target.value)}><option value="">Select election</option>{elections.map((e) => <option key={e._id || e.id} value={e._id || e.id}>{e.name} • {e.year} • {e.type}</option>)}</select></label>
      <label>Region<select value={regionId} onChange={(e) => setRegionId(e.target.value)}><option value="all">National — all regions</option>{regions.map((r) => <option key={r._id || r.id} value={r._id || r.id}>{r.name}</option>)}</select></label>
      <label>Constituency<select value={constituencyId} disabled={regionId === "all"} onChange={(e) => setConstituencyId(e.target.value)}><option value="all">All constituencies</option>{constituencies.map((c) => <option key={c._id || c.id} value={c._id || c.id}>{c.name}</option>)}</select></label>
      <label>Polling station<select value={pollingStationId} disabled={constituencyId === "all"} onChange={(e) => setPollingStationId(e.target.value)}><option value="all">All polling stations</option>{stations.map((s) => <option key={s.id || s._id} value={s.id || s._id}>{s.code} • {s.name}</option>)}</select></label>
    </div>
    ...
    <div className="stations">{loading ? <div className="empty">Loading assignments and result status…</div> : stations.length === 0 ? <div className="empty">No polling stations found for this geographic selection.</div> : stations.map((station) => {
      ...
      return <article className="station" key={station.id || station._id}>
