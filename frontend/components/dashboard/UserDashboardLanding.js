...
export default function UserDashboardLanding({ role = "user", title, navigation, activeSection = "overview", onSectionChange }) {
  ...
  return <DashboardShell role={role} navigation={navigation} activeSection={activeSection} onSectionChange={onSectionChange} mobileMenuOpen={mobileMenuOpen} onMobileMenuClose={() => setMobileMenuOpen(false)} user={user}>
    <main className="page">
      <header className="hero"><div><span>POLISYNC AFRICA • ACCOUNT WORKSPACE</span><h2>{user.displayName || user.firstName || roleTitle}</h2><p>{roleTitle} workspace with real account data and shared platform tools.</p></div></header>
