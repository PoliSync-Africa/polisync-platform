  ...
  const loadWeather = useCallback(async () => {
    if (typeof window === "undefined" || !navigator.geolocation) {
      ...
      setWeather((current) => ({
        ...
        condition: "Location services unavailable",
      }));
      return;
    }
    ...
    try {
      ...
      const hourly = buildHourlyForecast(data);
      const daily = buildDailyForecast(data);

      setWeather({
        loading: false,
        location: locationName || formatCoordinates(latitude, longitude),
        latitude,
        longitude,
        ...
        windUnit: currentUnits.wind_speed_10m || "km/h",
      });
    } catch (weatherError) {
      console.error("PoliSync weather error:", weatherError);

      if (weatherError?.code === 1) {
  ...
  return (
    <section className="polisync-weather-card" aria-live="polite">
        ...
        <div>
          ...
          <span>
            {weather.latitude !== null
              ...
              : "Current device location required"}
          </span>
        </div>
      </div>
      ...
      <div className="weather-temperature-label">
        Atmospheric air temperature at 2 m
      </div>
        ...
        <WeatherDetail
          ...
          value={
            weather.feelsLike !== null
              ? `${formatNumber(weather.feelsLike)}°C`
              : "--"
          }
        />
        <WeatherDetail
          ...
          <div className="hourly-grid">
            {visibleHourly.map((hour) => (
              <div className="hour-card" key={hour.time}>
                ...
                <strong>{formatNumber(hour.temperature)}°</strong>
            ...
            {weather.daily.map((day) => (
              <div className="daily-row" key={day.date}>
                ...
                <span className="daily-temp">
                  {formatNumber(day.max)}° / {formatNumber(day.min)}°
                </span>
      ...
      {error && (
        <div className="weather-error" role="alert">
          ...
          {permissionState === "denied" && (
            <span>
              Enable location permission for PoliSync in your browser/device
      ...
      <div className="weather-footer">
        <span className={error ? "weather-status-error" : "weather-status-live"}>
          <span className="weather-status-dot" />
          {refreshing ? "Updating live weather" : "Live weather"}
        </span>
      ...
      <style jsx>{`
        .polisync-weather-card {
          ...
          border-radius: 18px;
          background: #ffffff;
          box-shadow: 0 8px 24px rgba(17, 65, 36, 0.05);
        }
        ...
        .daily-row {
          display: grid;
          grid-template-columns: 52px 24px minmax(0, 1fr) auto 34px;
          align-items: center;
          gap: 7px;
          padding: 8px 9px;
        ...
        @media (max-width: 600px) {
          ...
          .daily-row {
            grid-template-columns: 48px 22px minmax(0, 1fr) auto 30px;
          }
...
function WeatherDetail({ icon, label, value }) {
  return (
    <div className="weather-detail">
      <span className="weather-detail-icon">{icon}</span>
...
function buildHourlyForecast(data) {
  ...
  return hourly.time.slice(startIndex, startIndex + 6).map((time, index) => {
    ...
    return {
      time,
      temperature: numberOrNull(hourly.temperature_2m?.[sourceIndex]),
      feelsLike: numberOrNull(
        hourly.apparent_temperature?.[sourceIndex]
      ),
      precipitationProbability: numberOrNull(
        hourly.precipitation_probability?.[sourceIndex]
      ),
      weatherCode: hourly.weather_code?.[sourceIndex] ?? null,
    };
  });
}
...
function buildDailyForecast(data) {
  ...
  return daily.time.map((date, index) => ({
    date,
    max: numberOrNull(daily.temperature_2m_max?.[index]),
    min: numberOrNull(daily.temperature_2m_min?.[index]),
    precipitationProbability: numberOrNull(
...
function weatherCodeToText(code) {
  ...
  return labels[numericCode] || "Current conditions";
}
