...
export default function OrganizationElectionManager({ title = "Election Management", organizationType = "organization" }) {
  const [elections, setElections] = useState([]), [editing, setEditing] = useState(null), [error, setError] = useState(""), [saving, setSaving] = useState(false);
  ...
  const load = async () => { try { setError(""); const d = await request("/api/elections"); setElections(d.elections || []); } catch (e) { setError(e.message); } };
  useEffect(() => { load(); }, []);
  ...
  const save = async e => { e.preventDefault(); setSaving(true); setError(""); try { if (editing) await request(`/api/elections/${editing}`, { method:"PATCH", body:JSON.stringify(form) }); else await request("/api/elections/create", { method:"POST", body:JSON.stringify(form) }); setForm(empty); setEditing(null); await load(); } catch (x) { setError(x.message); } finally { setSaving(false); } };
  ...
  return <section style={{display:"grid",gap:16}}>
    ...
    <form onSubmit={save} style={{display:"grid",gap:12,padding:16,border:"1px solid #dfe8e2",borderRadius:14,background:"#fff"}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}><button disabled={saving} type="submit" style={button}>{saving?"Saving…":editing?"Save Election Changes":"Create Election"}</button>{editing&&<button type="button" onClick={()=>{setEditing(null);setForm(empty)}} style={secondary}>Cancel</button>}</div>
    </form>
    ...
    <style jsx>{`input,select{width:100%;min-height:40px;border:1px solid #d5e0d8;border-radius:9px;padding:0 10px;color:#173d28;background:#fff;font-size:11px;box-sizing:border-box}button{font:inherit;cursor:pointer;border-radius:9px;padding:9px 12px;font-size:10px;font-weight:800}`}</style>
  </section>;
}
